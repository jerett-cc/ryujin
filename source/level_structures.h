...
namespace ryujin{
  namespace mgrit{
    ...
    LevelStructures<Description, dim, Number>::LevelStructures(
        const MPI_Comm &comm_x, const int refinement)
        ...
        , discretization(std::make_shared<Discretization>(
              comm_x, level_refinement, "/C - Discretization"))
        , offline_data(std::make_shared<OfflineData>(
              comm_x, *discretization, "/OfflineData"))
        , initial_values(std::make_shared<InitialValues>(
              *hyperbolic_system, *offline_data, "/E - InitialValues"))
        , hyperbolic_module(
        ...
        , time_integrator(
              std::make_shared<TimeIntegrator>(comm_x,
                                               ...
                                               *parabolic_module,
                                               "/H - TimeIntegrator"))
        , mesh_adaptor(std::make_shared<MeshAdaptor>(comm_x,
                                                     *offline_data,
                                                     *hyperbolic_system,
                                                     *parabolic_system,
                                                     "/I - MeshAdaptor"))
        , postprocessor(std::make_shared<Postprocessor>(
              comm_x, *offline_data, *hyperbolic_system, *parabolic_system,  "/J - VTUOutput"))
        ...
        , quantities(std::make_shared<Quantities>(
              comm_x, *offline_data, *hyperbolic_system, *parabolic_system, "/K - Quantities"))
    {}
    ...
    void LevelStructures<Description, dim, Number>::prepare(std::string base_name)
    {
      ...
      offline_data->prepare(problem_dimension, n_precomputed_values);
