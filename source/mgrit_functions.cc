...
namespace mgrit_functions{
  ...
  calculate_drag_and_lift(mgrit::MyApp *app, const mgrit::MyVector &u, const braid_Real t)
  {
    if (dim == 2) {
      ...
      // app->levels[app->finest_index]->offline_data->discretization().finite_element();
      // // the finite element const int degree =
      // app->levels[app->finest_index]->offline_data->discretization().finite_element().degree;
      // dealii::QGauss<1> face_quadrature_formula =
      // app->levels[app->finest_index]->offline_data->discretization().quadrature_1d();
      const int n_q_points = app->levels[app->finest_index]
