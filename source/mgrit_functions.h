...
namespace mgrit_functions
{
  ...
  /// @param app The MyApp object which knows about all the data structures at
  /// the level of MyVector.
  ...
  /// @tparam Number The numeric storage, e.g. double or float.
  /// @tparam Description The desctiption of the hyperbolic system, see ryujin.
  /// @param app The calling MyApp function which knows about problem dimension
  /// and data handlers.
  ...
  /// @brief This function is the dim=3 version of the above. It is not
  /// implemented.
  /// FIXME: change this brief if ever implemented.
  /// @tparam Number The numeric storage, e.g. double or float.
  /// @tparam Description The desctiption of the hyperbolic system, see ryujin.
  ...
  /// @brief This function figures out a globally averaged state. In other
  /// words, we calculate
  ///        \frac{\sum_i U_i}{N}, where N is the number of DOF on the global
  ///        mesh. If the mesh is distributed, we compute local averages, then
  ///        allreduce each component.
  ...
  /// @param u The solution state on the mesh that we want use to compute an
  /// average state.
  /// @param level The level that describes the vector.
  /// @param app The app containing the level structures we need to do work on
  /// u.
  template <typename Description, int dim, typename Number>
  dealii::
      Tensor<1, Description::HyperbolicSystemView::problem_dimension, Number>
  ...
  /// @param level The level that describes the vector we want to enforce
  /// physicality.
  /// @param app The app containing the level structures we need to do work on
  /// u.
  ...
  /// @brief This function measures the conserved quantities and entropy in the
  /// system,
  ...
  void conserved_and_entropy_in_system(
      ...
      const Number time);
  /// @brief This function senses whether or not the internal energy of the
  /// system exceeds a threshold and returns a bool when this happens. It has a
  /// side effect that if the threshold is exceeded, it prints to terminal that
  /// this has happened the (x,y,z) coordinates of the failure, as well as
  /// prints the data when it does.
  ...
  /// @param calling      An integer representing the calling function in
  /// xbraid.
  /// @param E_threshold  The maximum allowable threshold, above which we print
  /// information.
  template <typename Description, int dim, typename Number>
  ...
  /// @brief This function tests the admissibility of each DOFs data and returns
  /// true if all the DOFs pass, false otherwise.
  /// @tparam Description The description of the system, providing a pressure
  /// function and an entropy function.
  /// @tparam dim The spatial dimension.
  /// @tparam Number Either a double or float.
