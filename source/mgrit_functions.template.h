...
namespace mgrit_functions{
  ...
  // Specialize dim=2 for Navier Stokes Equation
  template <typename Number>
  dealii::Tensor<1, 2>
  calculate_drag_and_lift(mgrit::MyApp<Number, ryujin::NavierStokes::Description, 2> *app,
                          const mgrit::MyVector<Number, ryujin::NavierStokes, 2> &u,
                          [[maybe_unused]] const braid_Real t)
  {
    using scalar_type = dealii::LinearAlgebra::distributed::Vector<Number>;
    // In this function, dim = 2 is a valid assumption
    constexpr int dim = 2;
    ...
    const auto hyperbolic_system_view =
        app->levels[app->finest_level]
            ->hyperbolic_system->template view<2, Number>();
    const auto parabolic_system_view =
        app->levels[app->finest_level]
      ->hyperbolic_system->template view<2, Number>();//TODO
    ...
    std::vector<dealii::SymmetricTensor<2,dim>> velocity_gradients(n_q_points);
    ...
    for (int c = 0; c < dim; c++){
      momentum.at(c).reinit(offline_data->scalar_partitioner(),
                            mpi_communicator);
      velocity[c].reinit(offline_data->scalar_partitioner(),
			 mpi_communicator);
    }
    dealii::Tensor<1, dim> normal_vector;
    dealii::SymmetricTensor<2, dim> fluid_stress;
    ...
    // convert E to pressure, and get velocities
    for (unsigned int k = 0; k < offline_data->n_locally_owned(); k++) {
      ...
      for (int d = 0; d < dim; d++){
        m_square += std::pow(momentum.at(d).local_element(k), 2);
	velocity[d].local_element(k) = momentum[d].local_element(k)/rho;
      }
      // pressure = (gamma-1)*internal_energy
      pressure.local_element(k) =
          (hyperbolic_system_view.gamma() - 1.0) * (E - 0.5 * m_square / rho);
    ...
    double drag = 0.;
    double lift = 0.;
    // the dynamic viscosity is mu/rho, but here, density is not constant, so we
    // have a vector of dynamic viscosities.
    std::vector<Number> viscosity(offline_data->n_locally_owned());

    // calculate the viscosities
    for (int k = 0; k < offline_data->n_locally_owned(); k++)
      viscosity[k] = 
