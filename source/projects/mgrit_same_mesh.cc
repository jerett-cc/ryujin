...
int main(int argc, char *argv[])
{
  ...
  Assert(argc >= 1 /*program*/ + 1 /*parameter file*/ + 1 /*px*/ +
         ...
         dealii::ExcMessage(
             "You must provide the program with a parameter "
             "file, a number of spatial processors, a refinement, "
             "and a multigrid hierarcy of integrators. Here, the number of "
  ...
  const int refinement = std::stoi(argv[3]); // spatial refinement
  // a vector of names for the time integrator used on each level
  std::vector<std::string> integrator_levels(argc - 4);
  for (int i = 4; i < argc; i++)
    integrator_levels[i - 4] = argv[i];
  ...
  pout << "n_time in app: " << app.ntime << std::endl;
  // Run Simulation, set up the core variables.
